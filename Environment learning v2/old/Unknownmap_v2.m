%% Unknownmap Localcom V2
% UnKnownmap v1 but have some upgrades in node classification to work
% better in large map
...
float_thresh= MaxIt*1/3;        % float node condition
        ...
        while (1)
            ...
            if no_profit_move_counts(i) > float_thresh
                if rand() > 0.5
                    ...
                    al_pop(1,1:2) = pop(1,(i*2-1):(i*2)) + abs(phi).*[v v];
                end
                if rand() >0.5
                    fitness_ratio=0.995;             % adjust fitness ratio so that node can easily move
                else
                    fitness_ratio=0.999;
                end
            ...
            elseif no_move_counts(i) > trap_thresh
                ...
                if old_neigh_Cov-new_neigh_Cov>0.01*0.3
                    fitness_ratio=0.994;
